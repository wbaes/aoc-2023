    ...
    impl Hand {
        ...
        fn set_rank(&mut self) {
            ...
            if max == &5 {
                ...
                if let Some(_) = self.cards.chars().find(|x| x == &'1') {
                    let mut copy = card_amounts.clone();
                    copy.retain(|x| x == &3);
                    if copy.len() == 2 {
                        self.rank = 5; // full house with joker
                } else {
                    self.rank = 4; //three of a kind with joker
                }
        ...
        fn amount_of_cards(&self) -> Vec<i32> {
            ...
            for hand in self.cards.chars() {
                match hand {
                    ...
                    'Y' => card_amounts[1] += 1,
                    'X' => card_amounts[2] += 1,
                    '1' => card_amounts = card_amounts.iter().map(|c| c + 1).collect(), // Joker
                    'V' => card_amounts[4] += 1,
                    '9' => card_amounts[5] += 1,
                    '8' => card_amounts[6] += 1,
                    '7' => card_amounts[7] += 1,
            ...
            Ok(lines) => {
                for line in lines {
                    if let Ok(ip) = line {
                        ...
                        hands_bids.push(Hand {cards: split[0].replace("A", "Z").replace("K", "Y").replace("Q", "X").replace("J", "1").replace("T", "V").to_owned(), bid: split[1].parse().expect("Bid is not a number!"), rank: 0});
                    }
                }
